 ...
 * unsupportedArgs are recognized and ignored so they aren't confused for paths.
 * constructorArgs extra work (@see ./mocha/bin), these have been reimplemented.
 * @returns {{
 *  directory: string,
 *  opts: object,
 *  requires: array,
...
module.exports.getMochaOpts = function () {
  ...
  // even if provided, make sure they aren't confused for paths.
  var unsupportedArgs = {
    '-w': 'watch', '--watch': 'watch',
  ...
  var opts = contents
    ...
    .reduce(function (ret, item) {
      ...
      else if (!unsupportedArgs[optName]) {
        paths.push(optName);
      }
      return ret;
    }, []);
