 ...
 * @param {*} opts options for the reporter
 * @param {string} opts.directoryRegExp regular expression to parse out the current working directory.
 * @returns {function(object)} a reporter function that mocha can hook into.
 */
var Reporter = module.exports = function (opts) {
  directoryRegExp = new RegExp(opts.directoryRegExp, 'gi');
...
function cleanSuite (suite, includeTests) {
  ...
  var cleaned = {
    ...
    'title': suite.title,
    'fullFile': suite.file || '',
    'file': suite.file && suite.parent.root ? suite.file.replace(directoryRegExp, '') : '',
...
function cleanTest (test) {
  ...
  if (err && err.stack) {
    var stack = err.stack.replace(directoryRegExp, '');
